...
- Final review fix wave: on the gateway route `before_callbacks()` acts (guard, counter, grant) only for a request carrying the valid site token (`Aura_Worker_Security::token_matches()`); `walk()` walks `ArrayObject`/`ArrayIterator` through the view `json_encode()` emits; nesting past `MAX_WALK_DEPTH` (512) fails closed to the field placeholder; the counter sweep runs once an hour (final fix wave commit).
- PR #111 Codex round 2: the Slack receiver pattern also matches GovSlack's `hooks.slack-gov.com` (same `services|triggers|workflows` paths and boundary rules).
